...
/**
 * this class represents a dataset for the dataviewer, which either displays a
 * row or a column of a matrix. The number of the displayed column/row is
 * determined by the fixedIndex property. The type is determined by the
 * 
 * @author kfuchsbe
 */
public abstract class MatrixRowColDataSet extends ValidityMatrixDataSet {
	...
	/**
	 * an optional matrix, which will be added or subtracted from the original
	 * matrix, depending on the offsetSubtract - flag.
	 */
	private Matrix offsetMatrix;

	/**
	 * if set to true, the offset is subtracted from the matrix, otherwise it is
	 * added.
	 */
	...
	/**
	 * The first index of the row/col to display (inclusive). This might be
	 * <code>null</code>: in this case the first index is the first row/column
	 * in of the matrix.
	 */
	private Integer firstIndex = null;

	/**
	 * The last index of the row/col to display (exclusive, dataCount will be
	 * lastIndex-firstIndex). This might be <code>null</code>: In this case the
	 * last index is the last index of the matrix.
	 */
	 ...
	 * 
	 * @param type
	 *            the type of this dataset (row/column)
	 * @param name
	 ...
	 * 
	 * @param index
	 *            the index for which to find the row and column
	...
	/**
	 * @param m
	 *            the matrix for which to return the value
	 * @param rowCol
	 *            the rows/col for which to get the value
	 * @return the value
	 */
	private final static double getMatrixValue(Matrix m, RowCol rowCol) {
		if (rowCol != null) {
			if ((m != null) && (rowCol.row < m.getRowDimension())
					&& (rowCol.col < m.getColumnDimension())) {
	...
	/**
	 * this enum represents the two possible different types of a
	 * {@link MatrixRowColDataSet}: either it displayes one column, or one row.
	 * 
	 * @author kfuchsbe
	 */
