...
/**
 * 
 * Script console for BSF-compliant languages. The console is independent of the
 * scripting language, which can be changed dynamically. This class uses a
 * BSFManager, and not an Interpreter, as most other examples do.
 * 
 * NB: This class is inspired from bsh/util/JConsole.java by Patrick Niemeyer
 * (pat@pat.net). JConsole is subject to the Sun Public License version 1.0 and
 * to the GNU Lesser General Public License (the "LGPL").
 * http://cvs.sourceforge.
 * net/viewcvs.py/beanshell/BeanShell/src/bsh/util/JConsole.java
 * 
 * This is a small refactoring, which does not use the apache-bsf library
 * anymore, but the java-internal classes
 * 
 * @author Olivier Dameron (dameron@smi.stanford.edu), Patrick Niemeyer
 *         (pat@pat.net)
 * 
 */
public class ScriptConsole extends JScrollPane implements KeyListener,
	...
	private AttributeSet setStyle(Font font, Color color) {
		if (font != null)
			return setStyle(font.getFamily(), font.getSize(), color, font
					.isBold(), font.isItalic(), StyleConstants
	...
	public void mousePressed(MouseEvent event) {
		if (event.isPopupTrigger()) {
			menu
					.show((Component) event.getSource(), event.getX(), event
							.getY());
		}
	}
	...
	private synchronized void print(byte[] b, ConsoleOutputStream os) {
		try {
			doc.insertString(doc.getLength(), new String(b, 0, b.length),
