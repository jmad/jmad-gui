...
/**
 * A simple dialog to browse through yasp files in a directory and preview the
 * contents.
 * 
 * @author kfuchsbe
 * 
...
public class YaspFileBrowser extends JDialog {
	...
	private final static Logger logger = Logger
			.getLogger(YaspFileBrowser.class);
	...
	private void initComponents() {
		...
		table.getSelectionModel().addListSelectionListener(
				new FileSelectionAdapter(table));
	...
	private void parseDirectory() {
		...
		List<File> files = java.util.Arrays.asList(this.currentDir
				.listFiles(YaspFilters.TRAJECTORY_FILENAME_FILTER));
		tableModel.setFiles(files);
	}

	/**
	 * This class is the implementation of a listener to change the displayed
	 * Data in the {@link DataViewerPanel}.
	 * 
	 * @author kfuchsbe
	 * 
	 */
	private class FileSelectionAdapter implements ListSelectionListener {
		...
		public void valueChanged(ListSelectionEvent event) {
			if (event.getSource() == table.getSelectionModel()) {
				...
				if (index >= 0) {
					dvPanel.setYaspFile(tableModel.getFile(table
