 ...
 * 
 */
public abstract class PlotCreationPanel extends JPanel implements Titleable,
		Applyable {
	...
	private void initComponents() {
		...
		cboTfsDataSetType
				.setToolTipText("Selects the type of datasat (absolute, relative ...)");
		...
		this.xPanel = createVariableSelectionPanel(VarSelectionMode.SINGLE,
				MadxTwissVariable.S);
		varSelTabPane.addTab("x", xPanel);

		/* per default: create one y-panel */
		AllVarSelectionPanel yPanel = createVariableSelectionPanel(
				VarSelectionMode.MULTIPLE, null);
		...
		varSelTabPane.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				int tab = ui
						.tabForCoordinate(varSelTabPane, e.getX(), e.getY());
				if (tab >= 0) {
					JComponent selectedComponent = (JComponent) varSelTabPane
							.getComponentAt(tab);
	...
	private void addRemoveYPanel(boolean add) {
		...
		if (add) {
			/*
			 * if this is the first added, then we also have to remove the first
			 * one and re-add it with a different name.
			 */
			...
			yPanel = createVariableSelectionPanel(VarSelectionMode.MULTIPLE,
					null);
			this.yPanels.add(yPanel);
			...
			/*
			 * if there is only one left, then we also have to remove the first
			 * one and re-add it with a different name.
			 */
			if (this.yPanels.size() == 1) {
				varSelTabPane.removeTabAt(varSelTabPane.getTabCount() - 1);
	...
	private AllVarSelectionPanel createVariableSelectionPanel(
			VarSelectionMode varSelectionMode, TwissVariable defaultVariable) {
	...
	/**
	 * this method will be injected by spring to create a preconfigured
	...
	public boolean apply() {
		...
		tfsDataSetManager.createDataSets(txtName.getText(), xPanel
				.getSelectedVarManager().getSelectedVariable(), yVars,
				(TfsResultDataSetType) cboTfsDataSetType.getSelectedItem());
