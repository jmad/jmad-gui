/*
 * $Id: ModelTwissPanel.java,v 1.3 2009-03-16 16:36:33 kfuchsbe Exp $
 * 
 ...
 * 
 */
public class ModelTwissPanel extends JPanel implements
		EditHandlerUser<BeanTableEditHandler> {
	...
	private BeanTableModel doubleTableModel = new BeanTableModel(
			TwissInitialConditions.class);

	/** The table model for boolean values */
	private BeanTableModel booleanTableModel = new BeanTableModel(
			TwissInitialConditions.class);
	...
	private void initComponents() {
		...
		this.booleanTableModel.setValueClasses(new Class<?>[] { Boolean.class,
				boolean.class });
	 ...
	 * 
	 * @param component
	 *            the component, which shall be included in the scrollPane
	...
	/**
	 * set an (optional) edit handler to the double-table model. This can be
	 * used to select certain properties of the twiss, to allow to e.g. use them
	 * as variation parameters.
	 * 
	 * @param editHandler
	...
	public void setEditHandler(TablePanelEditHandler editHandler) {
		if (editHandler instanceof BeanTableEditHandler) {
			this.doubleTableModel
					.setEditHandler((BeanTableEditHandler) editHandler);
		}
	}
}
