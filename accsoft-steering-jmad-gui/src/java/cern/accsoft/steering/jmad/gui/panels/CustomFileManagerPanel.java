...
public class CustomFileManagerPanel extends JPanel {
	...
	private final static Logger logger = Logger
			.getLogger(CustomFileManagerPanel.class);

	/** the size of the table */
	private final static Dimension PREFERRED_TABLE_SIZE = new Dimension(200,
			200);
	...
	/**
	 * An instance of a preference class, which provides us with the default
	 * path to open files.
	 */
	...
	/**
	 * The manager, which keeps track of all available variables and strengthes
	 * from the custom files.
	 */
	private StrengthVarManager strengthVarManager;

	/**
	 * if this is set, then the file which is run is also parsed by the
	 * {@link StrengthVarManager} in order to be able to display the variables
	 * and plot them.
	 */
				...
				new ListSelectionListener() {
					...
					public void valueChanged(ListSelectionEvent e) {
						if (e.getSource() == table.getSelectionModel()) {
							...
							if (index >= 0) {
								tableModel.setCurrentElement(table
		...
		Action action = new AbstractAction("edit") {
			...
			public void actionPerformed(ActionEvent evt) {
				...
				if (editorCommand == null) {
					logger
							.warn("No editor configured! Cannot open the file to edit.");
					return;
				}
				String[] cmdArray = new String[] { editorCommand,
						selectedFile.getAbsolutePath() };
				try {
					Runtime.getRuntime().exec(cmdArray);
				} catch (IOException e) {
					logger.error("error executing command-array: "
							+ cmdArray.toString(), e);
				}
	...
	private Action createRunAction() {
		...
		action.putValue(AbstractAction.SHORT_DESCRIPTION,
				"runs the selected file within the current model.");
		return action;
	}
				...
				SwingUtilities.invokeLater(new Runnable() {
					...
					public void run() {
						CustomFileManagerPanel.this.tableModel
								.setFiles(CustomFileManagerPanel.this.customFileManager
										.getFiles());
					}
				});
			}
