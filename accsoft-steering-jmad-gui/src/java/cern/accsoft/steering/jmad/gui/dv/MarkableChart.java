/*
 * $Id: MarkableChart.java,v 1.2 2009-03-16 16:36:33 kfuchsbe Exp $
 * 
...
/**
 * This chart is a chart which listens on the MarkedElementsManager, in order to
 * display all the elements as markers.
 * 
 * @author kfuchsbe
 * 
 */
public class MarkableChart extends Chart {
	...
	/**
	 * with this map we keep track of the added markers, in order to be able to
	 * remove/replace them by name afterwards
	 */
	private Map<String, List<DataIndicator>> markerDataIndicators = new HashMap<String, List<DataIndicator>>();

	/**
	 * if this is set, then the x-positions of the markers are fetched from
	 * here. Otherwise the datasets are searched for a valid marker-position.
	 */
	private MarkerXProvider markerXProvider = null;

	/** the default font for an point indicator in the chart. */
	private final static Font DEFAULT_INDICATOR_FONT = new Font("Dialog",
			Font.BOLD, 10);

	/** the default style of the indicator in the chart */
	private static final Style DEFAULT_INDICATOR_STYLE = new Style(
			new BasicStroke(1, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 1,
					new float[] { 2, 2 }, 2), Color.GRAY, Color.BLACK);
	...
	private final static Font DEFAULT_FONT_RANGE = new Font("Dialog",
			Font.BOLD, 20);
	...
	/**
	 * the init method, which shall be called when the
	 * {@link MarkedElementsManager} was set. It creates all markers, according
	 * to the manager. {@link MarkedElementsManager} was set.
	 */
	...
	/**
	 * shows the marker for the given element name. This only works if at least
	 * one of the {@link DataSet}s implements the {@link MarkerXProvider}
	 * interface in order to determine the x-position of the marker.
	 * 
	 * @param elementName
	 */
	private void addMarker(String elementName) {
		/*
		 * we want to ensure, that only one indicator of the same name exists
	...
	/**
	 * removes the marker for the given elementName both from the chart and from
	 * our map.
	 * 
	 * @param elementName
	 *            the name of the element, for which to remove the marker.
	 */
	...
	private void createHVRangeIndicators() {
		...
		hRangeIndicator = createRangeIndicator(new DataRange(minX, hvBorder),
				COLOR_H_RANGE, "H");
		addDecoration(hRangeIndicator);

		vRangeIndicator = createRangeIndicator(new DataRange(hvBorder, maxX),
				COLOR_V_RANGE, "V");
	...
	private DataIndicator createRangeIndicator(DataRange dataRange,
			Color fillColor, String label) {
		DataIndicator rangeIndicator = new DataIndicator(DataIndicator.X_RANGE,
				dataRange, label);
	...
	/**
	 * searches for the first markerXProvider DataSet, that returns a non-null
	 * xposition for the given element. This is then returned.
	 * 
	...
	private List<Double> getMarkerXPositions(String elementName) {
		...
		/*
		 * if a x-provider is explicitely set, we try to get the x-pos from
		 * there.
		 */
		...
		/*
		 * if it is not set, or does not return a valid value, then we search in
		 * the datasets.
		 */
	...
	private DataIndicator createXIndicator(double xPos, String label) {
		DataIndicator pointIndicator = new DataIndicator(DataIndicator.X_VALUE,
				xPos, label);
	...
	/**
	 * @param markerXProvider
	 *            the markerXProvider to set
	 */
	...
	/**
	 * @param visibleHVIndicators
	 *            the visibleHVIndicators to set
	 */
