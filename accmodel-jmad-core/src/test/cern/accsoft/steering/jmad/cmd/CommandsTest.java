...
public class CommandsTest {
    ...
    public void testCallCommand() {
        ...
        assertEquals("call, file=\"" + file.getAbsolutePath() + "\";", command.compose());
    }
    ...
    public void testBeamCommand() {
        ...
        assertEquals("beam, particle=proton, energy=450.0, ex=1.6E-8, ey=1.4E-8, "
                + "sigt=0.077, sige=5.0E-4, npart=1.1E11, bv=1, sequence=ti2;", command.compose());
        ...
        command = new BeamCommand(beam);
        assertEquals("beam, particle=myparticle, mass=5.0E-4, charge=1.0, pc=20.5, "
                + "gamma=1.23, et=2.34, exn=3.45, eyn=4.56, kbunch=2808, bcurrent=1.5, " + "bunched;",
                command.compose());
    }

    @Test
    public void testTwissCommand() {
        TwissInitialConditionsImpl twiss = new TwissInitialConditionsImpl("test-twiss");
        ...
        assertEquals("twiss, deltap=0.0, betx=1.0, bety=5.0, alfx=0.2, alfy=0.6, "
                + "dx=3.0, dy=7.0, dpx=0.4, dpy=0.8, chrom;", command.compose());
    }

    @Test
    public void testAssignCommand() {
        command = new AssignCommand(file);
        assertEquals("assign, echo=\"" + file.getAbsolutePath() + "\";", command.compose());
    ...
    public void testDefineElement() {
        ...
        assertEquals(command.compose(), "testQuad: quadrupole, k1=1.0;");
    }
}
