 ...
 * @author kfuchsbe
 */
public class LhcThinModelDefinitionFactory extends
		AbstractLhcThinModelDefinitionFactory {
		
    @Override
    public JMadModelDefinition create() {
        JMadModelDefinitionImpl modelDefinition = new JMadModelDefinitionImpl();
        modelDefinition.setName(LhcUtil.THIN_MODEL_DEFINITION_NAME);

		modelDefinition.setModelPathOffsets(createModelPathOffsets());

		modelDefinition.addInitFile(new CallableModelFileImpl(
				"init-constants.madx", ModelFileLocation.RESOURCE));
		modelDefinition.addInitFile(new CallableModelFileImpl("V6.5.thin.seq"));
        ...
        SequenceDefinitionImpl lhcb1 = new SequenceDefinitionImpl(LhcUtil.SEQUENCE_NAME_BEAM1, createBeam(Direction.PLUS));
