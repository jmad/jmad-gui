/*
 * $Id: BeanTableModel.java,v 1.3 2009-01-19 17:13:40 kfuchsbe Exp $
 * 
...
/**
 * this class is a simple table model, which enables the display of an object,
 * that provides java-bean like properties in a table. the table then has as
 * many rows as the bean has properties and 2 columns: one for the name of the
...
public class BeanTableModel extends AbstractTableModel {
	...
	/**
	 * the classes, which are allowed for the value-column. One has to take
	 * care, that they are compatible, since table only supports one class for
	 * the column. To tell the table, which renderer to use, we use the first
	 * entry in this array.
	 * 
	 * By default we support Double and double.
	 */
	private Class<?>[] valueClasses = new Class<?>[] { Double.class,
			double.class };
	...
	public Object getValueAt(int row, int col) {
		if (col == COL_INDEX_SELECTED) {
			if (getEditHandler() != null) {
				return getEditHandler().getCheckValue(this.bean,
						getPropertyName(row));
			} else {
				return false;
			}
	...
	public void setValueAt(Object value, int row, int col) {
		...
		case COL_INDEX_SELECTED:
			if (getEditHandler() != null) {
				getEditHandler().setCheckValue(getBean(), getPropertyName(row),
						(Boolean) value);
			}
			break;
		case COL_INDEX_VALUE:
	...
	public boolean isCellEditable(int row, int col) {
		if (COL_INDEX_SELECTED == col) {
			return ((getEditHandler() != null) && (getEditHandler()
					.isEditable()));
	 ...
	 *            the value to set
	 * @param index
	 *            the index of the property to which to set the value
	 */
	private void setPropertyValue(Object value, int index) {
		...
		if (setter == null) {
			logger.error("no setter method for property '"
					+ getPropertyName(index) + "' could be found.");
			return;
		}
		...
		try {
			...
			logger.error("Error while invoking setter '" + setter.getName()
					+ "' of bean '" + getBean().toString(), e);
	 ...
	 * 
	 * @param index
	 *            the index for which to find the getter
	...
	private Method getSetter(int index) {
		...
		String setterName = SETTER_PREFIX
				+ getterName.substring(getGetterPrefix().length());
		Method setter = null;
		for (int i = 0; i < valueClasses.length; i++) {
			...
			try {
				...
				logger.error("Error while searching for method '" + setterName
						+ "'.", e);
	 ...
	 * 
	 * @param index
	 *            the index in the getterNames - list
	...
	private String getPropertyName(int index) {
		...
		String propertyName = getterName.substring(startIndex, startIndex + 1)
				.toLowerCase();
	...
	private Object getPropertyValue(int index) {
		...
		try {
			...
			logger.error("Error while invoking getter '" + method.getName()
					+ "' of bean '" + getBean().toString(), e);
		} catch (IllegalAccessException e) {
			logger.error("Error while invoking getter '" + method.getName()
					+ "' of bean '" + getBean().toString(), e);
	...
	/**
	 * searches for all getters in the bean. These are methods which begin with
	 * the prefix. Additionaly we only keep such, which return the classes which
	 * we support.
	 */
	private void initGetterNames() {
		...
		for (Method method : this.beanClass.getMethods()) {
			...
			if (methodName.startsWith(getGetterPrefix())
					&& methodName.length() > getGetterPrefix().length()) {
	 ...
	 * 
	 * @param bean
	 *            the bean, which shall be displayed.
	 */
